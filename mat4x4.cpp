...
{
	for (auto x = 0; x < MatrixDimensions(); x++)
	{
		for (auto y = 0; y < MatrixDimensions(); y++)
		{
			mData[x][y] = val;
		}
	}
...
{
	assert(col >= 0 && col <= MatrixDimensions());
	assert(row >= 0 && row <= MatrixDimensions());
...
void mat4x4::lookAt(vec3 eye, vec3 at, const vec3 &worldUp)
{
	setToIdentity();
...
{
	for (auto i = 0; i < MatrixDimensions(); i++)
	{
		for (auto j = 0; j < MatrixDimensions(); j++)
		{
			if (i == j)
				mData[i][j] = 1;
			else
...
//Vector4D mat4x4::operator *(Vector4D v)
//{
...
mat4x4 mat4x4::operator *(mat4x4 m)
{
	...
	for (auto i = 0; i < MatrixDimensions(); i++)
	{
		for (auto j = 0; j < MatrixDimensions(); j++)
		{
			for (auto k = 0; k < MatrixDimensions(); k++)
...
//
//vec3 mat4x4::extractLocation()
//{
//	return vec3(mData[0][3], mData[1][3], mData[2][3]);
//}
//
//vec3 mat4x4::extractRotation()
//{
